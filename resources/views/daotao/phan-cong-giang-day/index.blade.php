                ...
                <form action="{{ route('dao-tao.lop-hoc-phan.phan-cong.store', $lopHocPhan->id) }}" method="POST">
                    ...
                    <div class="row">
                        <div class="col-md-5">
                            ...
                            <select name="giang_vien_id" class="form-select @error('giang_vien_id') is-invalid @enderror" required>
                                <option value="">-- Chọn giảng viên --</option>
                                @foreach($giangViens as $gv)
                                    <option value="{{ $gv->id }}">{{ $gv->ma_giang_vien }} - {{ $gv->ho_ten }}</option>
                                @endforeach
                            </select>
                            @error('giang_vien_id')
                        ...
                        <tbody>
                            @forelse($lopHocPhan->lopHocPhanGiangVien as $index => $pc)
                                <tr>
                                    ...
                                    <td>{{ $pc->ngay_phan_cong ? date('d/m/Y', strtotime($pc->ngay_phan_cong)) : 'N/A' }}</td>
                                    <td>{{ $pc->nguoiPhanCong->name ?? 'N/A' }}</td>
                                    <td>{{ $pc->phan_cong_giang_day ?? '-' }}</td>
                                    <td>
                                        <form action="{{ route('dao-tao.phan-cong.destroy', $pc->id) }}" 
                                              method="POST" 
