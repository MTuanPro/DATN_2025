...
<body>
    <div class="header">
        ...
        <p>Học kỳ: {{ $hocKy->ten_hoc_ky }} - Năm học: {{ $hocKy->nam_hoc }}</p>
    </div>
    ...
    <div class="info">
        <table>
            <tr>
                ...
                <td width="25%">{{ $sinhVien->ma_sinh_vien }}</td>
                <td width="25%"><strong>Họ tên:</strong></td>
                <td width="25%">{{ $sinhVien->ho_ten }}</td>
            </tr>
            <tr>
                ...
                <tr>
                    ...
                    @for ($thu = 2; $thu <= 8; $thu++)
                        ...
                        @elseif($cell)
                            ...
                            @endphp
                            <td rowspan="{{ $rowspan }}"
                                class="{{ $loaiLop == 'ly_thuyet' ? 'ly-thuyet' : 'thuc-hanh' }}">
                                <div class="subject-name">{{ $lichHoc->lopHocPhan->monHoc->ten_mon }}</div>
                                <div class="subject-code">{{ $lichHoc->lopHocPhan->monHoc->ma_mon }}</div>
                                <div class="room">
                                    @if ($lichHoc->lichHocCoDinh && $lichHoc->lichHocCoDinh->phongHoc)
                                        Phòng: {{ $lichHoc->lichHocCoDinh->phongHoc->ten_phong }}
                                    @endif
                                </div>
                                <div class="teacher">
                                    @if ($lichHoc->lichHocCoDinh && $lichHoc->lichHocCoDinh->giangVien)
                                        GV: {{ $lichHoc->lichHocCoDinh->giangVien->ho_ten }}
                                    @endif
                                </div>
                            </td>
                        @else
