                    ...
                    <form action="{{ route('admin.quyen.store') }}" method="POST">
                        ...
                        <div class="form-group mb-3">
                            ...
                            <p class="text-muted small mb-2">
                                ...
                                VD: Quyền "Thêm khoa" chỉ nên áp dụng cho "Phòng đào tạo", không nên cho "Admin".
                            </p>
                                        ...
                                        @foreach ($actors as $key => $label)
                                            <div class="col-md-6">
                                                <div class="form-check mb-2">
                                                    <input class="form-check-input @error('actors') is-invalid @enderror"
                                                        type="checkbox" name="actors[]" value="{{ $key }}"
                                                        id="actor_{{ $key }}"
                                                        {{ in_array($key, old('actors', [])) ? 'checked' : '' }}>
                                                    <label class="form-check-label" for="actor_{{ $key }}">
                                                        {{ $label }}
                                                        @if ($key == 'admin')
