        ...
        @if ($lopHocPhan->trang_thai_lop === 'dang_hoc' && $lopHocPhan->ly_do_tra_ve)
            <section class="section">
                <div class="alert alert-warning alert-dismissible fade show" role="alert">
                    <h5 class="alert-heading"><i class="bi bi-exclamation-triangle"></i> Điểm đã được trả về để chỉnh sửa
                    </h5>
        ...
        @if (isset($daDuyetDiem) && $daDuyetDiem && $trangThaiLan2 === 'da_duyet')
            <section class="section">
                <div class="alert alert-success alert-dismissible fade show" role="alert">
                    ...
                    <p class="mb-0">Điểm đã được đào tạo duyệt cả 2 lần và công bố. Bạn không thể chỉnh sửa hoặc gửi lại
                        điểm sau khi đã duyệt.</p>
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                </div>
            </section>
        @endif
        ...
        @if ($daGuiLan2 && !$daDuyetDiem)
            <section class="section">
                <div class="alert alert-info alert-dismissible fade show" role="alert">
                    ...
                    <p class="mb-0">Bạn đã gửi điểm lần 2 (cuối kỳ). Sau khi đào tạo duyệt, điểm sẽ được công bố và bạn
                        không thể sửa nữa.</p>
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                </div>
            </section>
        ...
        @if (isset($daKetThuc) && $daKetThuc)
            <section class="section">
                <div class="alert alert-danger alert-dismissible fade show" role="alert">
                    ...
                    <p class="mb-0 mt-2"><strong>Bạn không thể sửa điểm sau khi lớp học phần đã kết thúc.</strong> Vui lòng
                        liên hệ phòng đào tạo nếu cần chỉnh sửa.</p>
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                </div>
            </section>
        @endif
                ...
                <div class="card-header d-flex justify-content-between align-items-center">
                    ...
                    <div class="d-flex gap-2">
                        @if (!$daKhoaDiem && $dangDienRa)
                            <a href="{{ route('giangvien.nhap-diem.download-template', $lopHocPhan->id) }}"
                        ...
                        @endif
                        @if ($trangThaiLan1 === 'da_duyet' || $trangThaiLan1 === 'da_gui')
                            <a href="{{ route('giangvien.nhap-diem.xuat-danh-sach-thi', $lopHocPhan->id) }}"
                ...
                <div class="card-body">
                    @if ($cauHinhs->isEmpty())
                        <div class="alert alert-warning">
                            <i class="bi bi-exclamation-triangle"></i> Chưa có cấu hình đầu điểm cho lớp này. Vui lòng liên
                            hệ phòng đào tạo.
                        </div>
                    @else
                            ...
                            @if ((!$daKhoaDiem && !isset($daDuyetDiem)) || (!$daDuyetDiem && !$daKhoaDiem && $dangDienRa))
                                ...
                                @if ($laGiangVienChinh && !$daGuiLan2)
                                    @php
                                        ...
                                        $lanGui = 1;
                                        if (
                                            $trangThaiLan1 === 'da_duyet' ||
                                            ($trangThaiLan1 === 'da_gui' && $trangThaiLan2 !== 'da_duyet')
                                        ) {
                                            $lanGui = 2;
                                        }
                                        $choPhepGui =
                                            ($lanGui == 1 && $choPhepGuiLan1) || ($lanGui == 2 && $choPhepGuiLan2);
                                    @endphp
                                    @if ($choPhepGui && !$daKhoaDiem)
                                        <button type="button" class="btn btn-primary"
                                            onclick="guiDiemChoDaoTao({{ $lanGui }})">
                                            <i class="bi bi-send"></i>
                                            Gửi điểm lần {{ $lanGui }} ({{ $lanGui == 1 ? 'giữa kỳ' : 'cuối kỳ' }})
                                        </button>
                                    @elseif(!$choPhepGui)
                                        <button type="button" class="btn btn-secondary" disabled
                                            title="Đào tạo chưa mở gửi điểm lần {{ $lanGui }}">
    ...
    @push('scripts')
        ...
        <script>
                ...
                cauHinhs.forEach(cauHinh => {
                    ...
                    for (let cot = 1; cot <= cauHinh.so_cot; cot++) {
                        const input = row.querySelector(
                            `input[data-cau-hinh-id="${cauHinh.id}"][data-cot-diem="${cot}"]`);
                        ...
                        .then(data => {
                            ...
                            if (data.success) {
                                ...
                                if (diemTKElement) {
                                    ...
                                    console.log('Updated diem TK for svId:', svId, 'from:', oldValue, 'to:', data
                                        .diem_tk, 'is_tam_thoi:', data.is_tam_thoi);
                                } else {
                ...
                rows.forEach(row => {
                    ...
                    inputs.forEach(input => {
                        ...
                        const data = {
                            ...
                            cau_hinh_id: parseInt(input.dataset.cauHinhId),
                            cot_diem: parseInt(input.dataset.cotDiem),
                            diem_so: value === '' || value === null || value === '-' ? null : parseFloat(
                                value)
                        };
            ...
            function guiDiemChoDaoTao(lanGui = null) {
                ...
                const text = 'Bạn có chắc muốn gửi điểm ' + lanGuiText +
                    ' cho đào tạo để duyệt? Sau khi gửi, bạn sẽ không thể sửa điểm cho đến khi đào tạo duyệt hoặc trả về.';
                                        ...
                                        Swal.fire({
                                            ...
                                            if (confirmResult.isConfirmed) {
                                                ...
                                                fetch('{{ route('giangvien.nhap-diem.gui-dao-tao', $lopHocPhan->id) }}', {
                                                    ...
                                                    .catch(error => {
                                                        console.error('Error:', error);
                                                        Swal.fire('Lỗi!', 'Có lỗi xảy ra khi gửi điểm',
                                                            'error');
                                                    });
                                            }
                                        });
