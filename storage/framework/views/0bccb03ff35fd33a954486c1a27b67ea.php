

<?php $__env->startSection('title', 'Nhập điểm'); ?>

<?php $__env->startSection('content'); ?>
    <div class="page-heading">
        <div class="page-title">
            <div class="row">
                <div class="col-12 col-md-6 order-md-1 order-last">
                    <h3>Nhập điểm</h3>
                    <p class="text-subtitle text-muted">Quản lý nhập điểm lớp học phần</p>
                </div>
                <div class="col-12 col-md-6 order-md-2 order-first">
                    <nav aria-label="breadcrumb" class="breadcrumb-header float-start float-lg-end">
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item"><a href="<?php echo e(route('giangvien.dashboard')); ?>">Dashboard</a></li>
                            <li class="breadcrumb-item active">Nhập điểm</li>
                        </ol>
                    </nav>
                </div>
            </div>
        </div>

        <section class="section">
            <div class="card">
                <div class="card-header">
                    <h4 class="card-title">Danh sách lớp học phần</h4>
                </div>
                <div class="card-body">
                    <?php if(session('success')): ?>
                        <div class="alert alert-success alert-dismissible fade show">
                            <?php echo e(session('success')); ?>

                            <button type="button" class="btn-close" data-bs-dismiss="alert"></button>
                        </div>
                    <?php endif; ?>

                    <?php if(session('error')): ?>
                        <div class="alert alert-danger alert-dismissible fade show">
                            <?php echo e(session('error')); ?>

                            <button type="button" class="btn-close" data-bs-dismiss="alert"></button>
                        </div>
                    <?php endif; ?>

                    <?php if($lopHocPhans->isEmpty()): ?>
                        <div class="alert alert-info">
                            <i class="bi bi-info-circle"></i>
                            Bạn chưa được phân công giảng dạy lớp học phần nào.
                        </div>
                    <?php else: ?>
                        <div class="table-responsive">
                            <table class="table table-hover">
                                <thead>
                                    <tr>
                                        <th>Mã lớp HP</th>
                                        <th>Tên lớp HP</th>
                                        <th>Môn học</th>
                                        <th>Học kỳ</th>
                                        <th>Vai trò</th>
                                        <th>Số SV</th>
                                        <th>Tiến độ</th>
                                        <th>Trạng thái</th>
                                        <th>Hành động</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <?php $__currentLoopData = $lopHocPhans; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $lhp): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        <tr>
                                            <td><strong><?php echo e($lhp['ma_lop_hp']); ?></strong></td>
                                            <td><?php echo e($lhp['ten_lop_hp']); ?></td>
                                            <td><?php echo e($lhp['mon_hoc']); ?></td>
                                            <td><?php echo e($lhp['hoc_ky']); ?></td>
                                            <td>
                                                <?php if($lhp['vai_tro'] === 'giang_vien_chinh'): ?>
                                                    <span class="badge bg-primary">Giảng viên chính</span>
                                                <?php else: ?>
                                                    <span class="badge bg-secondary">Giảng viên phụ</span>
                                                <?php endif; ?>
                                            </td>
                                            <td><?php echo e($lhp['tong_sv']); ?></td>
                                            <td>
                                                <div class="progress" style="height: 25px;">
                                                    <div class="progress-bar <?php echo e($lhp['ty_le'] >= 100 ? 'bg-success' : 'bg-warning'); ?>" 
                                                         role="progressbar" 
                                                         style="width: <?php echo e($lhp['ty_le']); ?>%">
                                                        <?php echo e($lhp['sv_co_diem']); ?>/<?php echo e($lhp['tong_sv']); ?> (<?php echo e($lhp['ty_le']); ?>%)
                                                    </div>
                                                </div>
                                            </td>
                                            <td>
                                                <?php if(isset($lhp['da_ket_thuc']) && $lhp['da_ket_thuc']): ?>
                                                    <span class="badge bg-dark">Kết thúc</span>
                                                <?php elseif($lhp['da_khoa_diem']): ?>
                                                    <span class="badge bg-danger">Đã khóa điểm</span>
                                                <?php else: ?>
                                                    <span class="badge bg-success">Đang mở</span>
                                                <?php endif; ?>
                                            </td>
                                            <td>
                                                <?php if(isset($lhp['da_ket_thuc']) && $lhp['da_ket_thuc']): ?>
                                                    <button class="btn btn-sm btn-secondary" disabled title="Lớp đã kết thúc">
                                                        <i class="bi bi-lock"></i> Đã kết thúc
                                                    </button>
                                                <?php else: ?>
                                                    <a href="<?php echo e(route('giangvien.nhap-diem.show', $lhp['id'])); ?>" 
                                                       class="btn btn-sm btn-primary">
                                                        <i class="bi bi-pencil-square"></i> Nhập điểm
                                                    </a>
                                                <?php endif; ?>
                                            </td>
                                        </tr>
                                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                </tbody>
                            </table>
                        </div>
                    <?php endif; ?>
                </div>
            </div>
        </section>
    </div>
<?php $__env->stopSection(); ?>

<?php echo $__env->make('layouts.layout-giangvien', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\Admin\DATN_2025\resources\views/giangvien/nhap-diem/index.blade.php ENDPATH**/ ?>